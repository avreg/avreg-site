...
if (!isset($refresh)) {
	...
	printf(
		'<IMG id="viewport" class="cam_snapshot" src="%s&width=%s&height=%s" style="border: 1px solid; %s" alt="%s снапшот" onerror="img_evt(1);"  />',
		$cam_url,
		$width,
		$heigt,
...
<script type="text/javascript">
	...
	var CAM_INFO = {
		...
		'active': <?php if ($cam_conf['work'] && $cam_conf['allow_networks']) {echo 'true';
} else {echo 'false';
} ?>,
		'width': <?php echo $w; ?>,
		'height': <?php echo $h; ?>,
