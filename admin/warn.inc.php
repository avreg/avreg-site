...
if ($ret !== false && $ret > 0) {
	$warnStr[$warnStr_cnt] = 'Обнаружен факт удаления файлов с диска вручную. Их список <a href="' . $conf['prefix'] . $conf['media-alias'] . '/' . $conf['removed-manually'] . '" target="_blank">' . $rmanfile . '</a> (' . filesizeHuman(
			$ret / 1024
		) . ')';
	$warnStr_cnt++;
}
