	...
	public function gallery_update_tree_events($start, $end, $cameras = false, $on_dbld_evnts = 'ignore')
	{
		...
		while ($res->fetchInto($line, DB_FETCHMODE_ASSOC)) {
			...
			if (in_array($line[$this->_key('EVT_ID')], array(15, 16, 17))) {
				...
				if (in_array($line[$this->_key('EVT_ID')], array(23, 12))) {
					$tree_events[$key]['VIDEO_COUNT']++;
					$tree_events[$key]['VIDEO_SIZE'] += $line[$this->_key('FILESZ_KB')];
				} else {
					if (in_array($line[$this->_key('EVT_ID')], array(32))) {
...
//      			'dbl_rows'=> $dbl_events,
	...
	public function add_layouts(
		...
		$query = sprintf(
			'INSERT INTO LOCAL_LAYOUTS (BIND_MAC, DISPLAY, MON_NR, MON_TYPE, MON_NAME, %s, CHANGE_HOST, CHANGE_USER) VALUES (\'local\', \'%s\', %d, \'%s\', \'%s\', %s, \'%s\', \'%s\')',
			implode(', ', $fWINS),
			$display,
			$mon_nr,
			$mon_type,
	...
	public function web_add_layouts(
		...
		$query = sprintf(
			'INSERT INTO WEB_LAYOUTS (BIND_MAC, MON_NR, MON_TYPE, SHORT_NAME, PRINT_CAM_NAME , PROPORTION, RECONNECT_TOUT, WINS, CHANGE_HOST, CHANGE_USER) '
